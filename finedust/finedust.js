/**
 * Fine Dust — mobile viewport lock + archive table toggle
 */
(function () {
    if (!document.body.classList.contains('page-finedust')) return;
    ...
    function lockViewportHeight() {
        if (!isMobile()) {
            document.documentElement.style.removeProperty('--finedust-vh');
            document.body.classList.remove('finedust-mobile-locked');
            resetColumns();
            return;
        }

        document.body.classList.add('finedust-mobile-locked');
        document.documentElement.style.setProperty('--finedust-vh', `${window.innerHeight}px`);
        window.scrollTo(0, 0);
        initColumns();
    }
    ...
    window.__finedustTextScrollReset = function () {
        resetColumns();
    };
    ...
    function initColumns() {
        ...
        document.querySelectorAll('.finedust-body-col').forEach((viewport) => {
            const track = viewport.querySelector('.finedust-body-col-track');
            if (!track) return;
    ...
    function blockDocumentTouch(e) {
        ...
        if (e.target.closest('.finedust-body-col')) return;
        if (e.target.closest('.finedust-archive')) return;
        e.preventDefault();
    }
    ...
    function initArchiveToggle() {
        const archive = document.getElementById('finedust-archive');
        const btn = archive && archive.querySelector('.finedust-scroll-btn');
        const panel = document.getElementById('finedust-archive-panel');
        ...
        const setOpen = (open) => {
            ...
            if (!next) {
                const preview = document.getElementById('finedust-row-preview');
                if (preview) {
        ...
        const isTextScrollArea = (target) =>
            Boolean(target && target.closest && target.closest('.finedust-body-col'));
        ...
        const isIgnoredScrollTarget = (target) => {
            ...
            if (target.closest('#finedust-archive-panel')) return true;
            return false;
        };
        ...
        document.addEventListener('click', (e) => {
            ...
            if (e.target.closest('#finedust-archive-panel')) return;
            if (e.target.closest('.finedust-scroll-btn')) return;
            if (e.target.closest('.about-lang-switch')) return;
            setOpen(false);
        });
    ...
    function initDateSort() {
        const table = document.querySelector('.finedust-table');
        const sortBtn = document.getElementById('finedust-date-sort');
        ...
        const applySort = () => {
            const head = table.querySelector('.finedust-table-row--head');
            const rows = Array.from(
                table.querySelectorAll('.finedust-table-row:not(.finedust-table-row--head)')
            );
    ...
    function initRowPreview() {
        ...
        document.querySelectorAll('.finedust-table-row[data-preview]').forEach((row) => {
    ...
    function initMobileRowNavigate() {
        const TAP_SLOP = 12;
        document.querySelectorAll('a.finedust-table-row--link[href]').forEach((link) => {
            let startX = 0;
            let startY = 0;
            let moved = false;
